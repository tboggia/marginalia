...
-- ------------------------------------------------------------------- grants
-- RLS filters rows. It does not grant access to a table, and it cannot: a role
-- needs the ordinary SQL privilege first, and only then do the policies above
-- decide which rows it gets. Both are required.
--
-- This used to be inherited and invisible. Supabase's default privileges for
-- schema `public` once handed anon and authenticated full CRUD on every table
-- created there, so a file like this one could define policies and never
-- mention a grant. On a current project they no longer do — a table created by
-- `postgres` in `public` comes out with TRUNCATE, REFERENCES and TRIGGER and
-- none of select, insert, update or delete. Every query from the app then dies
-- with a flat "permission denied for table documents", which looks nothing like
-- an RLS problem and is not fixed by touching a policy.
--
-- So they are written down here rather than inherited. Being explicit also
-- means this file no longer depends on which Supabase generation the project
-- was created in.
--
-- `anon` is granted nothing, deliberately. Every policy above requires
-- auth.uid(), so a signed-out client has no row it could reach anyway; refusing
-- the grant states that one level earlier and one level harder.
grant select, insert, update, delete
  on documents, memberships, progress, annotations
  to authenticated;
